"""
SNOMED CT GPFP 데이터를 Neo4j에 로드하는 스크립트

BCB_07_06.ipynb의 Neo4j 구조를 참고하여
SNOMED CT GPFP 데이터를 Neo4j에 구축합니다.
"""
...
class SNOMEDToNeo4jLoader:
    """SNOMED CT 데이터를 Neo4j에 로드하는 클래스"""
    ...
    def load_descriptions_to_neo4j(self):
        ...
        with self.driver.session() as session:
            for concept_id, descs in tqdm(self.descriptions.items(), desc="Description 추가"):
                # Preferred Term 찾기 (typeId가 900000000000013009)
                preferred_term = None
                all_terms = []
                ...
                # Preferred Term이 없으면 첫 번째 설명 사용
    ...
    def build_kg(self, clear_first: bool = True):
        """전체 KG 구축 프로세스"""
        print("="*60)
        print("SNOMED CT GPFP → Neo4j KG 구축 시작")
        print("="*60)
...
def main():
    ...
    loader = SNOMEDToNeo4jLoader(international_path, gpfp_path)
