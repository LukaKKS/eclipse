"""
SNOMED CT GPFP 데이터를 Neo4j에 로드하는 최적화된 스크립트

불필요한 메타데이터 속성을 제거하여 성능 최적화
"""
...
class SNOMEDToNeo4jLoaderOptimized:
    """SNOMED CT 데이터를 Neo4j에 로드하는 최적화된 클래스"""
    
    def __init__(self, international_path: str, gpfp_path: str, include_metadata: bool = False):
        self.international_path = international_path
        self.gpfp_path = gpfp_path
        self.include_metadata = include_metadata  # 메타데이터 포함 여부
    ...
    def load_relationships_to_neo4j(self):
        """Relationship 엣지를 Neo4j에 로드 (최적화 버전)"""
        print("\n=== Relationship 엣지 로드 ===")
    ...
    def get_statistics(self):
        ...
        with self.driver.session() as session:
            node_count = session.run("MATCH (n:Concept) RETURN count(n) AS count").single()['count']
            edge_count = session.run("MATCH ()-[r:RELATES_TO]->() RETURN count(r) AS count").single()['count']
    ...
    def build_kg(self, clear_first: bool = True):
        """전체 KG 구축 프로세스"""
        print("="*60)
        print("SNOMED CT GPFP → Neo4j KG 구축 (최적화 버전)")
        print("="*60)
        print(f"메타데이터 포함: {'예' if self.include_metadata else '아니오 (최적화)'}")
        print("="*60)
...
def main():
    ...
    loader = SNOMEDToNeo4jLoaderOptimized(
        international_path, 
        gpfp_path,
        include_metadata=False  # False로 설정하면 메타데이터 제외
    )
